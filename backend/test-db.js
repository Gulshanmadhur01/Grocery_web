...
async function verify() {
  ...
  if (db.categories.length < 8) {
    throw new Error(`Expected at least 8 categories, but found ${db.categories.length}`);
  }
  
  // Test Products
  console.log(`- Verified Products: Count = ${db.products.length}`);
  if (db.products.length < 14) {
    throw new Error(`Expected at least 14 seed products, but found ${db.products.length}`);
  }
